...
class NearestNeighborClassifier:
    ...
    def make_data(cls, x: list[list[float]], y: list[float]) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        """
        return torch.as_tensor(x, dtype=torch.float32), torch.as_tensor(y, dtype=torch.float32) # create 2 tensors from x and y
    ...
    def compute_data_statistics(cls, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        """
        return x.mean(dim=0, keepdim=True), x.std(dim=0, keepdim=True) # return mean and std dev of x
        raise NotImplementedError
    ...
    def input_normalization(self, x: torch.Tensor) -> torch.Tensor:
        ...
        """
        return (x - self.data_mean) / self.data_std # return normalized x
    ...
    def get_nearest_neighbor(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        """
        # raise NotImplementedError
        x = self.input_normalization(x) # normalize x 
        distances = torch.norm(self.data_normalized - x, dim=1) # compute euclidean distance
        idx = torch.argmin(distances) # get index of nearest neighbor
        return self.data[idx], self.label[idx] # return neighbor and label
    ...
    def get_k_nearest_neighbor(self, x: torch.Tensor, k: int) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        idx = torch.topk(distances, k, largest=False).indices # get k nearest neighbors
        return self.data[idx], self.label[idx] # return neighbors data and labels
    ...
    def knn_regression(self, x: torch.Tensor, k: int) -> torch.Tensor:
        ...
        """
        _, labels = self.get_k_nearest_neighbor(x, k) # get the labels of the neighbors
        return labels.mean() # return the average
        raise NotImplementedError
