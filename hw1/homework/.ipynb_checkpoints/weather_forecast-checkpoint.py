...
class WeatherForecast:
    ...
    def find_min_and_max_per_day(self) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        """
        return self.data.min(dim=1).values, self.data.max(dim=1).values # return min and max per day
    ...
    def find_the_largest_drop(self) -> torch.Tensor:
        ...
        """
        daily_avg = self.data.mean(dim=1) # calculate average
        return torch.diff(daily_avg).min() # return the most negative change
        raise NotImplementedError
    ...
    def find_the_most_extreme_day(self) -> torch.Tensor:
        ...
        """
        daily_mean = self.data.mean(dim=1, keepdim=True)  # compute mean per day (keeping shape)
        deviations = (self.data - daily_mean).abs()  # compute absolute value of deviation from mean
        max_deviation_idx = deviations.argmax(dim=1)  # find index of max deviation per row (day)
        return self.data.gather(1, max_deviation_idx.unsqueeze(1)).squeeze(1)  # return extreme
        raise NotImplementedError
    ...
    def max_last_k_days(self, k: int) -> torch.Tensor:
        ...
        Returns:
            tensor of size (k,)
        """
        return self.data[-k:].max(dim=1).values # return max value for each day (last 10 days)
        raise NotImplementedError
    ...
    def predict_temperature(self, k: int) -> torch.Tensor:
        ...
        """
        return self.data[-k:].mean() # return the mean of the last k days
        raise NotImplementedError
    ...
    def what_day_is_this_from(self, t: torch.FloatTensor) -> torch.LongTensor:
        ...
        """
        distances = (self.data - t).abs().sum(dim=1) # take the sum of absolute differences
        return torch.argmin(distances).long() # return the index of most similar day
        raise NotImplementedError
